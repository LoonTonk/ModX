...
/*         $player_array = array();
        foreach ($players as $player_id => $player) { // Used for testing
    ...
    function updateBoardOnPattern($patternTokens, $board) {
        ...
        foreach ($newScores as $player_or_team => $newScore) { // First pass finds the players/teams who gained points
            ...
            if ($scoreChange > 0) {
                ...
                if ($this->getGameStateValue('ffa_or_teams') == 2) {
                    ...
                    $scoreGainMessage .= sprintf(self::_('team %s scores %d point(s)'), $teamName, $scoreChange);
                } else {
        ...
        foreach ($newScores as $player_or_team => $newScore) { // Second pass finds the players/teams who lost points
            ...
            if ($scoreChange < 0) {
                ...
                if ($this->getGameStateValue('ffa_or_teams') == 2) {
                    $teamName = self::TEAM_NAMES[(int)$player_or_team];
                    $scoreLossMessage .= sprintf(self::_('team %s loses %d point(s)'), $teamName, -$scoreChange);
                } else {
                    $playerName = $this->getPlayerNameById($player_or_team);
        ...
        foreach($patterns as $index => $pattern) {
            self::notifyAllPlayers( 'scorePattern', $pattern_message, array(
                'x' => $centerToken['x'],
                'y' => $centerToken['y'],
                'patternCode' => $pattern,
                'player_id' => $player_id,
...
//////////// 
    ...
    /*
        ...
        (note: each method below must match an input method in modx.action.php)
    */
