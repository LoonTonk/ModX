...
class ModX extends Gamegui
{
	...
	override setup(gamedatas: Gamedatas): void
	{
		...
		for( var player_id in gamedatas.players ) {
				...
				const teamNum = gamedatas.playerTeams[parseInt(player_id)];
				...
				if (teamNum !== '0') {
					...
					this.addTooltip(`team_icon_${player_id}`,team_icon_tt , '');
				}
	...
	*/
	...
	notif_playToken( notif: NotifAs<'playToken'> )
	{
		...
		if (id > ModX.MAX_WILDS && id !== undefined) { // It is a player token
...
// The global 'bgagame.modx' class is instantiated when the page is loaded. The following code sets this variable to your game class.
dojo.setObject( "bgagame.modx", ModX );
// Same as: (window.bgagame ??= {}).modx = ModX;